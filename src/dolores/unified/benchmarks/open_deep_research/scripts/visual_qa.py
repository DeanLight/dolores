...
def encode_image(image_path):
    if image_path.startswith("http"):
        ...
        download_path = os.path.join(tempfile.gettempdir(), fname)
...
def visualizer(image_path: str, question: str | None = None) -> str:
    ...
    payload = {
        "model": "gpt-5-nano", # NOTE: changed to newer model
        "messages": [
            {
                "role": "user",
                "content": [
        ...
        "max_completion_tokens": 1000,  # NOTE: changed from `max_tokens` because it is no longer supported
    }
